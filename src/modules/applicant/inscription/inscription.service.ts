...
export class InscriptionService {
	...
	async createApplicantInscription(user: applicant) {
		try {
			const inscription =
	...
	async updatePaymentMethod(
		...
		try {
			const { applicant_payment_inscription } =
	...
	async updateChangePaymentStatus(user: applicant) {
		try {
			...
			if (payment_status === true) {
				...
				throw new BadRequestException(
					`No se a procesado el pago de la inscripcion`,
				);
			}
		} catch (error) {
			if (
	...
	async getInscriptonData(user: applicant) {
		try {
			...
			if (!payment_token_inscription) {
				throw new NotFoundException(
					'No se encontró información de la solicitud de inscripción',
				);
			}
